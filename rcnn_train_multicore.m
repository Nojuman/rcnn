...
caches = cell(length(imdb.class_ids), 1);
...
for hard_epoch = 1:max_hard_epochs
  for i = 1:length(imdb.image_ids)
    ...
    num_classes = length(imdb.class_ids);
    assert(isequal(imdb.class_ids, 1:num_classes), ...
           'Class ids must go from 1 to N for parallel training');
    new_detectors_W = cell(num_classes, 1);
    new_detectors_B = cell(num_classes, 1);
    new_SVs_keys_neg = cell(num_classes, 1);
    new_SVs_scores_neg = cell(num_classes, 1);
    retrained = false(num_classes, 1);
       
    % Add sampled negatives to each classes training cache, removing
    % duplicates
    parfor (j = 1:num_classes, 2)
      caches_j = caches{j};
      if ~isempty(keys{j})
        if ~isempty(caches_j.keys_neg)
          [~, ~, dups] = intersect(caches_j.keys_neg, keys{j}, 'rows');
          assert(isempty(dups));
        end
        caches_j.X_neg = cat(1, caches_j.X_neg, X{j});
        caches_j.keys_neg = cat(1, caches_j.keys_neg, keys{j});
        caches_j.num_added = caches_j.num_added + size(keys{j},1);
      end
      ...
      hit_retrain_limit = (caches_j.num_added > caches_j.retrain_limit);
      if (first_time || hit_retrain_limit || is_last_time) && ...
          ~isempty(caches_j.X_neg)
        retrained(j) = true;
        fprintf('>>> Updating %s detector <<<\n', imdb.classes{j});
        fprintf('Cache holds %d pos examples %d neg examples\n', ...
                size(caches_j.X_pos,1), size(caches_j.X_neg,1));
        [new_w, new_b] = update_model(caches_j, opts);
        new_detectors_W{j} = new_w;
        new_detectors_B{j} = new_b;
%         rcnn_model.detectors.W(:, j) = new_w;
%         rcnn_model.detectors.B(j) = new_b;
        caches_j.num_added = 0;

        z_pos = caches_j.X_pos * new_w + new_b;
        z_neg = caches_j.X_neg * new_w + new_b;

        caches_j.pos_loss(end+1) = opts.svm_C * opts.pos_loss_weight * ...
                                    sum(max(0, 1 - z_pos));
        caches_j.neg_loss(end+1) = opts.svm_C * sum(max(0, 1 + z_neg));
        caches_j.reg_loss(end+1) = 0.5 * new_w' * new_w + ...
                                    0.5 * (new_b / opts.bias_mult)^2;
        caches_j.tot_loss(end+1) = caches_j.pos_loss(end) + ...
                                    caches_j.neg_loss(end) + ...
                                    caches_j.reg_loss(end);

        for t = 1:length(caches_j.tot_loss)
          fprintf('    %2d: obj val: %.3f = %.3f (pos) + %.3f (neg) + %.3f (reg)\n', ...
                  t, caches_j.tot_loss(t), caches_j.pos_loss(t), ...
                  caches_j.neg_loss(t), caches_j.reg_loss(t));
        end

        % store negative support vectors for visualizing later
        SVs_neg = find(z_neg > -1 - eps);
        new_SVs_keys_neg{j} = caches_j.keys_neg(SVs_neg, :);
        new_SVs_scores_neg{j} = z_neg(SVs_neg);
%         rcnn_model.SVs.keys_neg{j} = caches{j}.keys_neg(SVs_neg, :);
%         rcnn_model.SVs.scores_neg{j} = z_neg(SVs_neg);

        % evict easy examples
        easy = find(z_neg < caches_j.evict_thresh);
        caches_j.X_neg(easy,:) = [];
        caches_j.keys_neg(easy,:) = [];
...
%         fprintf('  %d neg support vectors\n', numel(find(z_neg > -1 - eps)));
    ...
    % Go back in serial and stuff things back into the model.
    if isempty(rcnn_model.detectors.W)
        rcnn_model.detectors.W = zeros(length(new_detectors_W{1}), num_classes);
        rcnn_model.detectors.B = zeros(1, num_classes);
    end
    for j = 1:num_classes
%         disp(j);
%         disp(size(rcnn_model.detectors.W));
%         disp(size(new_detectors_W{j}));
%         disp(retrained(j));
        if retrained(j)
            rcnn_model.detectors.W(:, j) = new_detectors_W{j};
            rcnn_model.detectors.B(j) = new_detectors_B{j};
            rcnn_model.SVs.keys_neg{j} = new_SVs_keys_neg{j};
            rcnn_model.SVs.scores_neg{j} = new_SVs_scores_neg{j};
        end
    end
    first_time = false;
...
switch solver
  case 'liblinear'
    ll_opts = sprintf('-q -w1 %.5f -c %.5f -s %d -B %.5f', ...
